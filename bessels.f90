    ...
    !Feb 2009: minor fix for non-_flat compiled with non-smart IF evaluation
    !Dec 2011: minor tweak to DoRecurs for smoother errors across _flat for L~O(30)
    !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    !_Flat bessel function module
    ...
    ! parameters for working out where the _flat Bessel functions are small
    ...
    public phi_recurs,phi_langer, bjl, Bessels_Free
    ...
    if (DebugMsgs .and. FeedbackLevel > 0) write (*,*) 'Generating _flat Bessels...'
    ...
    ! uniform first-order asymptotic approximation in the open, _closed     c
    ! and _flat cases. This approximation is EXCELLENT for all lSamp%l >= 3.      c
    !                                                                      c
    ! The routine qintegral calculates the _closed-form answer              c
    ...
    !This allows _closed functions to be computed where chi << turning point
    ...
    ! recursion relations and _closed-form expressions for l=0,1.           *
    ...
    ! using the _closed form solution for l=1. (See, e.g., Numerical        *
    ...
    ! For _closed case, find equivalent chi in [0,pi/2]
    !
    if(K==1) then
    ...
    !
    ! _Closed form solution for K=1 and beta = l+1 (lowest eigenfunction)
    !
    ...
    ! For _closed case, find corrected eigenvalue beta
    !
    if(K==1) then
    ...
    !                                                                       c
    ! Note that for the _closed case, the variable arg must be between 0     c
